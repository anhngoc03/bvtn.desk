// ==========================================================
// BAO CAO NGHIEM THU (Sheet BC_nghiemthu)
// - Chi khoa/phong SO HUU de an moi duoc nop.
// - DIEU KIEN: DS_Deansangkien.Duyet_Khongduyet phai la "Duyệt" moi cho nop.
// - Toi da 1 file PDF / de an. Admin bat/tat nhan, cho sua khi con mo.
// ==========================================================

const BCNGHIEMTHU_STATUS_PROP_KEY = "BCNGHIEMTHU_STATUS";

function getBCNghiemThuStatus(token) {
  requireUser(token);
  const raw = PropertiesService.getScriptProperties().getProperty(BCNGHIEMTHU_STATUS_PROP_KEY);
  if (raw === null) return true;
  return raw === "true";
}

function setBCNghiemThuStatus(token, isOpen) {
  const user = requireUser(token);
  if (!isAdmin(user)) {
    return { success: false, message: "Chỉ Admin mới có quyền bật/tắt nhận báo cáo nghiệm thu." };
  }
  PropertiesService.getScriptProperties().setProperty(BCNGHIEMTHU_STATUS_PROP_KEY, isOpen ? "true" : "false");
  return { success: true, message: "Đã cập nhật.", status: !!isOpen };
}

function getBCNghiemThuDetail(token, khoaPhong, tenDeAn, maDeAn) {
  const user = requireUser(token);
...
// Kiem tra de an da duoc "Duyệt" chua (dieu kien bat buoc de nop BC_nghiemthu)
function isDeAnDaDuyet_(khoaPhong, tenDeAn) {
  const deanData = getSheetData(CONFIG.SHEET_DEAN);
  if (deanData.length < 2) return false;
  const deanMap = headerIndexMap(deanData[0]);
  if (!("Duyet_Khongduyet" in deanMap) || !("Khoaphong" in deanMap) || !("Tendean" in deanMap)) return false;

  const row = deanData.slice(1).find(function (r) {
    return String(r[deanMap["Tendean"]] || "").trim() === tenDeAn &&
           splitKhoaPhongValues_(r[deanMap["Khoaphong"]]).indexOf(khoaPhong) !== -1;
  });
  if (!row) return false;
  return String(row[deanMap["Duyet_Khongduyet"]] || "").trim() === "Duyệt";
}

function submitBCNghiemThu(token, data) {
  const user = requireUser(token);
  ...
  if (!isDeAnDaDuyet_(khoaPhong, tenDeAn)) {
    return { success: false, message: "Đề án/sáng kiến này chưa được duyệt, chưa thể nộp báo cáo nghiệm thu." };
  }
  ...
  if (!isAdmin(user) && !getBCNghiemThuStatus(token)) {
    return { success: false, message: "Chức năng hiện đang tạm ngừng nhận báo cáo nghiệm thu. Vui lòng liên hệ Admin." };
  }

  const sheet = getSheet(CONFIG.SHEET_BCNGHIEMTHU);
  ...
  if (f && f.data) {
    try {
      const folder = DriveApp.getFolderById(getDriveFolderIds_().BC_NGHIEM_THU);
      const bytes = Utilities.base64Decode(f.data);
      const blob = Utilities.newBlob(bytes, "application/pdf", f.name || "Bao_cao_nghiem_thu.pdf");
  ...
  invalidateSheetCache(CONFIG.SHEET_BCNGHIEMTHU);
